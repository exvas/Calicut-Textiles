        ...
        setTimeout(function() {
            ...
            if (printFormatField) {
                ...
                printFormatField.$wrapper.append('<div class="format-info">' +
                    '📝 This print format will be used to generate the PDF purchase invoice. Default is set from Whatsapp Settings.' +
                    '</div>');
            ...
            if (letterheadField) {
                ...
                letterheadField.$wrapper.append('<div class="format-info">' +
                    '💡 This letterhead will be used in the generated PDF purchase invoice. Default is set from Whatsapp Settings.' +
                    '</div>');
        ...
        callback: function(pf_response) {
            ...
            frappe.call({
                ...
                callback: function(lh_response) {
                    ...
                    frappe.call({
                        method: 'calicut_textiles.calicut_textiles.events.purchase_invoice.get_whatsapp_settings',
...
function show_saved_files_dialog(frm) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.purchase_invoice.get_saved_invoice_files',
        args: {
        ...
        callback: function(r) {
            if (r.message && r.message.success) {
                ...
                if (files.length > 0) {
                    ...
                    files_html = '<p class="text-muted">No saved PDF files found for this purchase invoice.</p>';
                }

                var files_dialog = frappe.msgprint({
                    title: 'Purchase Invoice PDFs',
                    message: files_html,
                    wide: true
                });
...
function update_resend_preview(frm, dialog, file_name) {
    ...
    var message_lines = [
        ...
        "Date: " + frappe.datetime.str_to_user(purchase_invoice.posting_date),
        "Amount: " + purchase_invoice.currency + " " + format_currency(purchase_invoice.grand_total, purchase_invoice.currency),
        "",
        "Supplier: " + (purchase_invoice.supplier_name || purchase_invoice.supplier),
    ];
    ...
    if (option === 'PDF for Manual Attachment') {
        message_lines.push("📄 Please find your purchase invoice PDF attached below.");
    } else {
        message_lines.push("Download purchase invoice: [PDF Link will be added here]");
    }
...
function send_existing_pdf_via_whatsapp(frm, mobile_number, option, file_name, file_url, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.purchase_invoice.resend_existing_pdf_whatsapp',
        args: {
            docname: frm.doc.name,
...
function get_supplier_mobile(supplier_name, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.purchase_invoice.get_supplier_mobile',
        args: {
...
function send_whatsapp_with_attachment(frm, mobile_number, print_format, letterhead, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.purchase_invoice.send_pdf_to_whatsapp',
        args: {
            docname: frm.doc.name,
            mobile_number: mobile_number,
...
function update_message_preview_enhanced(frm, dialog) {
    ...
    var message_lines = [
        ...
        "📋 *Purchase Invoice Details:*",
        "Purchase Invoice: #" + purchase_invoice.name,
        "Date: " + frappe.datetime.str_to_user(purchase_invoice.posting_time),
        "Amount: " + purchase_invoice.currency + " " + format_currency(purchase_invoice.grand_total, purchase_invoice.currency),
        "",
        "Supplier: " + (purchase_invoice.supplier_name || purchase_invoice.supplier),
    ];
    ...
    if (mobile_number) {
        preview_html += '<div style="margin-top: 10px; color: #666; font-size: 12px;">' +
            ...
            '📁 PDF will be saved to: <code>/files/Invoice_' + purchase_invoice.name + '.pdf</code>' +
            '</div>';
    }
...
function send_whatsapp_invoice(frm, mobile_number, print_format, letterhead, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.purchase_invoice.send_invoice_whatsapp',
        args: {
            docname: frm.doc.name,
            mobile_number: mobile_number,
            print_format: print_format,
