        ...
        setTimeout(function() {
            ...
            if (printFormatField) {
                ...
                printFormatField.$wrapper.append('<div class="format-info">' +
                    '📝 This print format will be used to generate the PDF purchase receipt. Default is set from Whatsapp Settings.' +
                    '</div>');
            ...
            if (letterheadField) {
                ...
                letterheadField.$wrapper.append('<div class="format-info">' +
                    '💡 This letterhead will be used in the generated PDF purchase receipt. Default is set from Whatsapp Settings.' +
                    '</div>');
        ...
        callback: function(pf_response) {
            ...
            frappe.call({
                ...
                callback: function(lh_response) {
                    ...
                    frappe.call({
                        method: 'calicut_textiles.calicut_textiles.events.purchase_receipt.get_whatsapp_settings',
...
function show_saved_files_dialog(frm) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.purchase_receipt.get_saved_invoice_files',
        args: {
        ...
        callback: function(r) {
            if (r.message && r.message.success) {
                ...
                if (files.length > 0) {
                    ...
                    files_html = '<p class="text-muted">No saved PDF files found for this purchase receipt.</p>';
                }

                var files_dialog = frappe.msgprint({
                    title: 'Purchase Receipt PDFs',
                    message: files_html,
                    wide: true
                });
...
function update_resend_preview(frm, dialog, file_name) {
    ...
    var message_lines = [
        ...
        "Date: " + frappe.datetime.str_to_user(purchase_receipt.posting_date),
        "Amount: " + purchase_receipt.currency + " " + format_currency(purchase_receipt.grand_total, purchase_receipt.currency),
        "",
        "Supplier: " + (purchase_receipt.supplier_name || purchase_receipt.supplier),
    ];
    ...
    if (option === 'PDF for Manual Attachment') {
        message_lines.push("📄 Please find your purchase receipt PDF attached below.");
    } else {
        message_lines.push("Download purchase receipt: [PDF Link will be added here]");
    }
...
function send_existing_pdf_via_whatsapp(frm, mobile_number, option, file_name, file_url, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.purchase_receipt.resend_existing_pdf_whatsapp',
        args: {
            docname: frm.doc.name,
...
function get_supplier_mobile(supplier_name, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.purchase_receipt.get_supplier_mobile',
        args: {
...
function send_whatsapp_with_attachment(frm, mobile_number, print_format, letterhead, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.purchase_receipt.send_pdf_to_whatsapp',
        args: {
            docname: frm.doc.name,
            mobile_number: mobile_number,
...
function send_whatsapp_invoice(frm, mobile_number, print_format, letterhead, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.purchase_receipt.send_invoice_whatsapp',
        args: {
            docname: frm.doc.name,
            mobile_number: mobile_number,
            print_format: print_format,
