...
function show_saved_files_dialog(frm) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.sales_invoice.get_saved_invoice_files',
        args: {
        ...
        callback: function(r) {
            if (r.message && r.message.success) {
                ...
                if (files.length > 0) {
                    ...
                    files_html = '<p class="text-muted">No saved PDF files found for this invoice.</p>';
                }
...
function update_resend_preview(frm, dialog, file_name) {
    ...
    var message_lines = [
        ...
        "Date: " + frappe.datetime.str_to_user(invoice.posting_date),
    ...
    if (invoice.due_date && invoice.outstanding_amount > 0) {
        message_lines.push("Due Date: " + frappe.datetime.str_to_user(invoice.due_date));
...
function send_existing_pdf_via_whatsapp(frm, mobile_number, option, file_name, file_url, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.sales_invoice.resend_existing_pdf_whatsapp',
        args: {
            docname: frm.doc.name,
...
function get_customer_mobile(customer_name, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.sales_invoice.get_customer_mobile',
        args: {
...
function send_whatsapp_with_attachment(frm, mobile_number, print_format, letterhead, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.sales_invoice.send_pdf_to_whatsapp',
        args: {
            docname: frm.doc.name,
            mobile_number: mobile_number,
...
function update_message_preview_enhanced(frm, dialog) {
    ...
    if (mobile_number) {
        preview_html += '<div style="margin-top: 10px; color: #666; font-size: 12px;">' +
            ...
            '📁 PDF will be saved to: <code>/files/Invoice_' + invoice.name + '.pdf</code>' +
            '</div>';
    }
...
function send_whatsapp_invoice(frm, mobile_number, print_format, letterhead, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.sales_invoice.send_invoice_whatsapp',
        args: {
            docname: frm.doc.name,
            mobile_number: mobile_number,
            print_format: print_format,
