erpnext.utils.BarcodeScanner = class BarcodeScanner {
	...
	update_table(data) {
		return new Promise((resolve, reject) => {
			...
			let fetch_item_code = Promise.resolve(item_code);
				...
				if (batch_no) {
					...
					frappe.db.get_value('Batch', { 'name': batch_no }, ['custom_qty', 'batch_qty'], (r) => {
						if (r ) {
							...
							this.show_alert(__("Unable to retrieve batch quantity for {0}", [batch_no]), "red");
							reject();
						}
					});
				} else {
					...
					frappe.run_serially([
						...
						() => this.set_item(row, resolved_item_code, barcode, batch_no, serial_no).then((qty) => {
							this.show_scan_message(row.idx, resolved_item_code, qty);
						}),
