frappe.ui.form.on('Sales Order', {
    ...
    customer: function(frm){
        if (frm.doc.customer) {
            get_customer_mobile(frm.doc.customer);
        }
        frappe.call({
...
function show_whatsapp_options_dialog(frm) {
    ...
    get_print_formats_letterheads_and_settings(function(print_formats, letterheads, current_print_format, current_letterhead) {
        ...
        var dialog = new frappe.ui.Dialog({
            ...
            secondary_action_label: 'Get Customer Mobile',
            secondary_action: function() {
                get_customer_mobile(frm.doc.customer, dialog);
            }
        });
        ...
        setTimeout(function() {
            ...
            if (printFormatField) {
                ...
                printFormatField.$wrapper.append('<div class="format-info">' +
                    '📝 This print format will be used to generate the PDF sales order. Default is set from Whatsapp Settings.' +
                    '</div>');
            ...
            if (letterheadField) {
                ...
                letterheadField.$wrapper.append('<div class="format-info">' +
                    '💡 This letterhead will be used in the generated PDF sales order. Default is set from Whatsapp Settings.' +
                    '</div>');
        ...
        callback: function(pf_response) {
            ...
            frappe.call({
                ...
                callback: function(lh_response) {
                    ...
                    frappe.call({
                        method: 'calicut_textiles.calicut_textiles.events.sales_order.get_whatsapp_settings',
...
function show_saved_files_dialog(frm) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.sales_order.get_saved_invoice_files',
        args: {
        ...
        callback: function(r) {
            if (r.message && r.message.success) {
                ...
                if (files.length > 0) {
                    ...
                    files_html = '<p class="text-muted">No saved PDF files found for this sales order.</p>';
                }

                var files_dialog = frappe.msgprint({
                    title: 'Sales Order PDFs',
                    message: files_html,
                    wide: true
                });
...
function update_resend_preview(frm, dialog, file_name) {
    ...
    var message_lines = [
        ...
        "Date: " + frappe.datetime.str_to_user(sales_order.transaction_date),
        "Amount: " + sales_order.currency + " " + format_currency(sales_order.grand_total, sales_order.currency),
        "",
        "Customer: " + (sales_order.customer_name || sales_order.customer),
    ];
    ...
    if (option === 'PDF for Manual Attachment') {
        message_lines.push("📄 Please find your sales order PDF attached below.");
    } else {
        message_lines.push("Download Sales Order: [PDF Link will be added here]");
    }
...
function send_existing_pdf_via_whatsapp(frm, mobile_number, option, file_name, file_url, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.sales_order.resend_existing_pdf_whatsapp',
        args: {
            docname: frm.doc.name,
...
function get_customer_mobile(customer_name, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.sales_order.get_customer_mobile',
        args: {
...
function send_whatsapp_with_attachment(frm, mobile_number, print_format, letterhead, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.sales_order.send_pdf_to_whatsapp',
        args: {
            docname: frm.doc.name,
            mobile_number: mobile_number,
...
function update_message_preview_enhanced(frm, dialog) {
    ...
    if (mobile_number) {
        preview_html += '<div style="margin-top: 10px; color: #666; font-size: 12px;">' +
            ...
            '📁 PDF will be saved to: <code>/files/SO_' + sales_order.name + '.pdf</code>' +
            '</div>';
    }
...
function send_whatsapp_invoice(frm, mobile_number, print_format, letterhead, dialog) {
    ...
    frappe.call({
        method: 'calicut_textiles.calicut_textiles.events.sales_order.send_invoice_whatsapp',
        args: {
            docname: frm.doc.name,
            mobile_number: mobile_number,
            print_format: print_format,
