...
def get_gl_entries(filters, accounting_dimensions):
	...
	# Add customer name to select fields
	customer_name_field = ""
	if filters.get("party_type") == "Customer":
		customer_name_field = """, 
			CASE 
				WHEN party_type = 'Customer' THEN (SELECT customer_name FROM tabCustomer WHERE name = party)
				ELSE NULL 
			END as customer_name"""
	
	select_fields += customer_name_field
...
def get_columns(filters):
	...
	columns += [
		...
		{"label": _("Party"), "fieldname": "party", "width": 100},
		{
			...
			"width": 200,
			"hidden": 1 if filters.get("party_type") != "Customer" else 0
		},
