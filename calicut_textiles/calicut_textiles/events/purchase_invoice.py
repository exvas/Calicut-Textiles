...
def send_invoice_whatsapp(docname, mobile_number, print_format=None, letterhead=None):
    """
    Generate Purchase Invoice PDF, save it to files, and create WhatsApp message link
    """
    try:
        ...
        # Check permissions
        if not frappe.has_permission("Purchase Invoice", "read", purchase_invoice.name):
            frappe.throw(_("No permission to access this Purchase Invoice"))

        # Generate and save PDF/HTML
        pdf_data = generate_and_save_invoice_pdf(purchase_invoice, print_format, letterhead)

        # Generate WhatsApp message
        whatsapp_data = create_whatsapp_message(mobile_number, purchase_invoice, pdf_data)
        ...
        frappe.log_error(f"WhatsApp send error: {str(e)}", "WhatsApp Purchase Invoice Error")
        return {
            "success": False,
            "error": str(e)
        }
...
def send_pdf_to_whatsapp(docname, mobile_number, print_format=None, letterhead=None):
    """
    Generate Purchase Invoice PDF for manual WhatsApp attachment
    """
    try:
        # Get the Purchase Receipt invoice document
        purchase_invoice = frappe.get_doc("Purchase Invoice", docname)
        ...
        whatsapp_data = create_whatsapp_message_for_attachment(mobile_number, purchase_invoice, pdf_data)
...
def resend_existing_pdf_whatsapp(docname, mobile_number, option, file_name, file_url):
    ...
    """
    try:
        # Get the purchase invoice order document
        purchase_invoice = frappe.get_doc("Purchase Invoice", docname)
...
def generate_and_save_invoice_pdf(purchase_invoice, custom_print_format=None, custom_letterhead=None):
    """
    ...
    try:
        ...
        # Save the file
        file_name = f"Invoice_{purchase_invoice.name}.{file_extension}"
        file_doc = save_file(
            fname=file_name,
...
Purchase Invoice Number: {purchase_invoice.name}
Supplier: {purchase_invoice.supplier_name or purchase_invoice.supplier}
Date: {purchase_invoice.posting_time}
Amount: {purchase_invoice.currency} {purchase_invoice.grand_total}
...
Please contact support for assistance.
            ...
            file_name = f"Invoice_{purchase_invoice.name}_fallback.txt"
            file_doc = save_file(
                fname=file_name,
...
def create_whatsapp_message(mobile_number, purchase_invoice, pdf_data):
    """Create WhatsApp message URL with purchase invoice details and PDF link"""
    try:
        ...
        message_lines = [
            "Good day! 👋",
            "",
            "Thank you for your interest in our purchase invoice! 🛍️",
            "",
            "📋 *Purchase Invoice Details:*",
            f"Purchase: #{purchase_invoice.name}",
            f"Date: {purchase_invoice.get_formatted('posting_time')}",
            f"Amount: {purchase_invoice.currency} {purchase_invoice.get_formatted('grand_total')}",
            "",
            f"Supplier: {purchase_invoice.supplier_name or purchase_invoice.supplier}",
        ]
        ...
        message_lines.extend([
            "",
            f"📄 Download Purchase Invoice: {full_pdf_url}",
            "",
...
def create_whatsapp_message_for_attachment(mobile_number, purchase_invoice, pdf_data):
    ...
    message_lines = [
        ...
        "",
        "📋 *Purchase Invoice Details:*",
        f"Purchase Invoice: #{purchase_invoice.name}",
        f"Date: {purchase_invoice.get_formatted('posting_date')}",
        f"Amount: {purchase_invoice.currency} {purchase_invoice.get_formatted('grand_total')}",
        "",
        f"Supplier: {purchase_invoice.supplier_name or purchase_invoice.supplier}",
    ]
...
def get_saved_invoice_files(docname):
    """Get all saved PDF files for the purchase invoice"""
    try:
        # Get all files attached to this Purchase Receipt
