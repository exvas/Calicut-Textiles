...
def send_invoice_whatsapp(docname, mobile_number, print_format=None, letterhead=None):
    """
    Generate Payment Entry PDF, save it to files, and create WhatsApp message link
    """
    try:
        ...
        # Check permissions
        if not frappe.has_permission("Payment Entry", "read", payment_entry.name):
            frappe.throw(_("No permission to access this Payment Entry"))

        # Generate and save PDF/HTML
        pdf_data = generate_and_save_invoice_pdf(payment_entry, print_format, letterhead)

        # Generate WhatsApp message
        whatsapp_data = create_whatsapp_message(mobile_number, payment_entry, pdf_data)
        ...
        frappe.log_error(f"WhatsApp send error: {str(e)}", "WhatsApp Payment Entry Error")
        return {
            "success": False,
            "error": str(e)
        }
...
def send_pdf_to_whatsapp(docname, mobile_number, print_format=None, letterhead=None):
    """
    Generate Payment Entry PDF for manual WhatsApp attachment
    """
    try:
        # Get the Payment Entry document
        payment_entry = frappe.get_doc("Payment Entry", docname)
        ...
        whatsapp_data = create_whatsapp_message_for_attachment(mobile_number, payment_entry, pdf_data)
...
def generate_and_save_invoice_pdf(payment_entry, custom_print_format=None, custom_letterhead=None):
    """
    Generate PDF for the Payment Entry using Whatsapp Settings configuration or custom parameters
    ...
    try:
        ...
        # Save the file
        file_name = f"Payment Entry_{payment_entry.name}.{file_extension}"
        file_doc = save_file(
            fname=file_name,
            content=file_content,
            dt="Payment Entry",
            dn=payment_entry.name,
        ...
        # Fallback: Create a simple text file with Payment Entry details
        try:
            settings = frappe.get_single("Whatsapp Settings")
            print_format = custom_print_format if custom_print_format is not None else (settings.whatsapp__print_format or "Standard")
            letterhead = custom_letterhead if custom_letterhead is not None else settings.whatsapp_letter_head
...
Payment Entry Number: {payment_entry.name}
Customer: {payment_entry.customer_name or payment_entry.party_name}
Date: {payment_entry.transaction_date}
Amount: {payment_entry.currency} {payment_entry.grand_total}
...
Please contact support for assistance.
            ...
            file_name = f"Payment Entry_{payment_entry.name}_fallback.txt"
            file_doc = save_file(
                fname=file_name,
...
def create_whatsapp_message(mobile_number, payment_entry, pdf_data):
    """Create WhatsApp message URL with Payment Entry details and PDF link"""
    try:
        ...
        message_lines = [
            "Good day! 👋",
            "",
            "Thank you for your interest in our Payment Entry! 🛍️",
            "",
            "📋 *Payment Entry Details:*",
            f"Payment Entry: #{payment_entry.name}",
            f"Date: {payment_entry.get_formatted('posting_date')}",
            f"Amount: {payment_entry.currency} {payment_entry.get_formatted('paid_amount')}",
            "",
            f"Customer: {payment_entry.party_name or payment_entry.party}",
        ]

        # if Payment Entry.valid_till:
        #     message_lines.append(f"Valid Till: {quotation.get_formatted('valid_till')}")

        message_lines.extend([
            "",
            f"📄 Download Payment Entry: {full_pdf_url}",
            "",
...
def create_whatsapp_message_for_attachment(mobile_number, payment_entry, pdf_data):
    ...
    message_lines.extend([
        "",
        "📄 Please find your Payment Entry attached below.",
        "",
...
def get_saved_invoice_files(docname):
    """Get all saved PDF files for the Payment Entry"""
    try:
        # Get all files attached to this Sales Payment Entry
        files = frappe.get_all("File",
            filters={
                "attached_to_doctype": "Payment Entry",
                "attached_to_name": docname,
                "file_name": ["like", "Payment Entry_%"]
            },
